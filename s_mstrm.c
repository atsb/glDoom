...
dboolean    bFileOpen = false, bPlaying = false, bBuffersPrepared = false;
dboolean    bPaused = false, bLooped = false;
...
static dboolean InitApp( HINSTANCE );
static dboolean InitInstance( HINSTANCE, int );
...
/*****************************************************************************/
static dboolean InitApp( HINSTANCE hInstance )
    {
    WNDCLASS    wc;
...
/*                                       */
/*****************************************************************************/
static dboolean InitInstance( HINSTANCE hInstance, int nCmdShow )
    {
    HWND    hWnd;
    RECT    crect;
    UINT    uCharsRead;
 ...
 */
    ...
    if (( mmrRetVal = midiStreamOpen( &hStream, &uMIDIDeviceID, (DWORD)1, (DWORD)MidiProc, (DWORD)0, CALLBACK_FUNCTION )) != MMSYSERR_NOERROR )
       {
...
LRESULT CALLBACK MainWindowProc( HWND hWnd, unsigned uMsg, WPARAM wParam, LPARAM lParam )
   {
    ...
    switch( uMsg )
             ...
             switch( LOWORD( wParam ))
                {
                 case IDM_FILE_OPEN:
                       ...
                       */
                       ...
                       /* BAL - "return of true indicates..."  - moron!
                        * A return of false indicates that the user did not select a filename.
                           ...
                           if ( bFileOpen )
                              {
                               ...
                               // BAL - Or you could just put it in a function you moron...
                           ...
                           if ( StreamBufferSetup())
                              {
                               ...
                               // Only IDIOTS take a true return and call it an ERROR...
                               // So, what play-school DOES Microsoft recruit from?
                               break;
                              }
                           else
                              {
                 ...
                 case IDM_FILE_CLOSE:
                      SendMessage( hWnd, WM_COMMAND, MAKEWPARAM( IDC_STOP, MSTREAM_STOPF_NOREOPEN ), 0L );
                      BuildTitleBarText();
                      break;
                 ...
                 case IDC_PLAY:
                      ...
                      if ( bPaused )
                         {
                          SendMessage( hWnd, WM_COMMAND, MAKEWPARAM( IDC_PAUSE, 0 ), 0L );
                          break;
                         }
                      // Clicking play while playing will restart from scratch
                      if ( bPlaying )
                         {
                          // Stop the file, allowing it to be reset so that we
                          // can start it over again from the beginning
                          SendMessage( hWnd, WM_COMMAND, MAKEWPARAM( IDC_STOP, 0 ), 0L );
                         }
       ...
       case WM_INITMENU:
            if ( bFileOpen )
               {
                EnableMenuItem( GetMenu( hWnd ), IDM_ACTIONS_PLAY, MF_BYCOMMAND | MF_ENABLED );
                EnableMenuItem( GetMenu( hWnd ), IDM_FILE_CLOSE, MF_BYCOMMAND | MF_ENABLED );
               }
            else
               {
                EnableMenuItem( GetMenu( hWnd ), IDM_ACTIONS_PLAY, MF_BYCOMMAND | MF_GRAYED );
                EnableMenuItem( GetMenu( hWnd ), IDM_FILE_CLOSE, MF_BYCOMMAND | MF_GRAYED );
               }
            if ( bLooped )
               {
                EnableMenuItem( GetMenu( hWnd ), IDM_ACTIONS_LOOPED,MF_BYCOMMAND | MF_ENABLED );
               }
            else
               {
                EnableMenuItem( GetMenu( hWnd ), IDM_ACTIONS_LOOPED, MF_BYCOMMAND | MF_GRAYED );
               }
            if ( bPlaying )
               {
                EnableMenuItem( GetMenu( hWnd ), IDM_ACTIONS_PAUSE, MF_BYCOMMAND | MF_ENABLED );
                EnableMenuItem( GetMenu( hWnd ), IDM_ACTIONS_STOP, MF_BYCOMMAND | MF_ENABLED );
               }
            else
               {
                EnableMenuItem( GetMenu( hWnd ), IDM_ACTIONS_PAUSE, MF_BYCOMMAND | MF_GRAYED );
                EnableMenuItem( GetMenu( hWnd ), IDM_ACTIONS_STOP, MF_BYCOMMAND | MF_GRAYED );
               }
            break;
       ...
       case WM_HSCROLL:
            ...
            if (((HWND)lParam == hWndVol) && bFileOpen )
               {
       ...
       case WM_DESTROY:
            ...
            if ( hStream )
               {
                SendMessage( hWnd, WM_COMMAND, MAKEWPARAM( IDC_STOP, MSTREAM_STOPF_NOREOPEN ), 0 );
               }
            FreeBuffers();
            if ( hStream )
               {
...
/*                                                                           */
/*****************************************************************************/
dboolean CALLBACK DLG_About( HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam )
    {
    switch( msg )
        {
        case WM_INITDIALOG:
    ...
    if ( bBuffersPrepared )
       {
        for ( idx = 0; idx < NUM_STREAM_BUFFERS; idx++ )
           {
            if (( mmrRetVal = midiOutUnprepareHeader( (HMIDIOUT)hStream, &ciStreamBuffers[idx].mhBuffer, sizeof(MIDIHDR))) != MMSYSERR_NOERROR )
               {
                MidiErrorMessageBox( mmrRetVal );
               }
...
/*****************************************************************************/
dboolean StreamBufferSetup( void )
   {
    int     nChkErr;
    dboolean    bFoundEnd = false;
    ...
    if (( mmrRetVal = midiStreamProperty( hStream, (LPBYTE)&mptd, MIDIPROP_SET | MIDIPROP_TIMEDIV )) != MMSYSERR_NOERROR )
       {
        MidiErrorMessageBox( mmrRetVal );
        ConverterCleanup();
        return( true );
    ...
    for ( nCurrentBuffer = 0; nCurrentBuffer < NUM_STREAM_BUFFERS; nCurrentBuffer++ )
       {
        ...
        if (( nChkErr = ConvertToBuffer( dwConvertFlag, &ciStreamBuffers[nCurrentBuffer] )) != CONVERTERR_NOERROR )
           {
        ...
        if ( !bBuffersPrepared )
           {
            if (( mmrRetVal = midiOutPrepareHeader( (HMIDIOUT)hStream, &ciStreamBuffers[nCurrentBuffer].mhBuffer, sizeof(MIDIHDR))) != MMSYSERR_NOERROR )
               {
                MidiErrorMessageBox( mmrRetVal );
                ConverterCleanup();
                return( true );
        ...
        if (( mmrRetVal = midiStreamOut( hStream, &ciStreamBuffers[nCurrentBuffer].mhBuffer, sizeof(MIDIHDR))) != MMSYSERR_NOERROR )
           {
            MidiErrorMessageBox( mmrRetVal );
            break;
           }
             ...
             if ( uCallbackStatus == STATUS_WAITINGFOREND )
                {
                 if ( nEmptyBuffers < NUM_STREAM_BUFFERS )
                    ...
                    {
                     uCallbackStatus = STATUS_CALLBACKDEAD;
                     PostMessage( hWndMain, WM_COMMAND, MAKEWPARAM( IDC_STOP, 0 ), 0L );
             ...
             if ( bPlaying && nEmptyBuffers )
                {
                 ...
                 if (( nChkErr = ConvertToBuffer( 0, &ciStreamBuffers[nCurrentBuffer] )) != CONVERTERR_NOERROR )
                    {
                     if ( nChkErr == CONVERTERR_DONE )
                        {
