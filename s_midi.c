...
static dboolean GetInFileData( LPVOID lpDest, DWORD cbToGet );
static dboolean GetTrackByte( PINTRACKSTATE ptsTrack, LPBYTE lpbyByte );
static dboolean GetTrackEvent( PINTRACKSTATE ptsTrack, PTEMPEVENT pteTemp );
static dboolean GetTrackVDWord( PINTRACKSTATE ptsTrack, LPDWORD lpdw );
static dboolean RefillTrackBuffer( PINTRACKSTATE ptsTrack );
static dboolean RewindConverter( void );
...
//
dboolean ConverterInit( LPSTR szInFile )
   {
    dboolean        fRet = true;
    ...
    if ( hInFile == INVALID_HANDLE_VALUE )
       {
        ...
        MessageBox( GetActiveWindow(), szTemp, szAppTitle, MB_OK | MB_ICONEXCLAMATION );
        goto Init_Cleanup;
       }
    ...
    // - file header itself
    //  
    if ( GetInFileData( &dwTag, sizeof(DWORD)) || ( dwTag != MThd ) || GetInFileData( &cbHeader, sizeof(DWORD)) ||
        (( cbHeader = DWORDSWAP( cbHeader )) < sizeof(MIDIFILEHDR)) || GetInFileData( &Header, cbHeader ) )
       {
        MessageBox( GetActiveWindow(), szInitErrInFile, szAppTitle, MB_OK | MB_ICONEXCLAMATION );
        goto Init_Cleanup;
       }
    ...
    //
    ifs.pitsTracks = (PINTRACKSTATE)GlobalAllocPtr( GPTR, ifs.dwTrackCount * sizeof(INTRACKSTATE));
    if ( ifs.pitsTracks == NULL )
       {
        MessageBox( GetActiveWindow(), szInitErrMem, szAppTitle, MB_OK | MB_ICONEXCLAMATION );
        goto Init_Cleanup;
       }

    for ( idx = 0, ptsTrack = ifs.pitsTracks; idx < ifs.dwTrackCount; ++idx, ++ptsTrack )
       {
        if (( ptsTrack->pTrackStart = GlobalAllocPtr( GHND, TRACK_BUFFER_SIZE )) == NULL )
           {
            MessageBox( GetActiveWindow(), szNoTrackBuffMem, szAppTitle, MB_OK | MB_ICONEXCLAMATION );
            goto Init_Cleanup;
           }

        if ( GetInFileData( &dwTag, sizeof(dwTag)) || ( dwTag != MTrk ) || GetInFileData( &cbHeader, sizeof(cbHeader)))
           {
            MessageBox( GetActiveWindow(), szInitErrInFile, szAppTitle, MB_OK | MB_ICONEXCLAMATION );
            goto Init_Cleanup;
           }
        ...
        ptsTrack->foTrackStart = SetFilePointer( hInFile, 0, NULL, FILE_CURRENT );

        if ( ptsTrack->dwTrackLength > TRACK_BUFFER_SIZE )
            dwToRead = TRACK_BUFFER_SIZE;
        else
            dwToRead = ptsTrack->dwTrackLength;
        if ( !ReadFile( hInFile, ptsTrack->pTrackStart, dwToRead, &cbRead, NULL ) || ( cbRead != dwToRead ))
           {
            MessageBox( GetActiveWindow(), szInitErrInFile, szAppTitle, MB_OK | MB_ICONEXCLAMATION );
            goto Init_Cleanup;
           }
        ...
        ptsTrack->foNextReadStart = SetFilePointer( hInFile, 0, NULL, FILE_CURRENT );
        ...
        // track's data
        SetFilePointer( hInFile, ptsTrack->foTrackStart + ptsTrack->dwTrackLength, NULL, FILE_BEGIN );
...
static dboolean GetInFileData( LPVOID lpDest, DWORD cbToGet )
   {
    ...
    if ( !ReadFile( hInFile, lpDest, cbToGet, &cbRead, NULL ) || ( cbRead != cbToGet ))
       {
        return( true );
       }
...
int ConvertToBuffer( DWORD dwFlags, LPCONVERTINFO lpciInfo )
   {
    ...
    if ( dwStatus & CONVERTF_STATUS_STUCK )
       {
        return( CONVERTERR_STUCK );
       }
    else
    if ( dwStatus & CONVERTF_STATUS_GOTEVENT )
       {
    ...
    for ( ; ; )
       {
        ...
        for ( idx = 0, ptsTrack = ifs.pitsTracks; idx < ifs.dwTrackCount; ++idx, ++ptsTrack )
           {
            if (( !( ptsTrack->fdwTrack & ITS_F_ENDOFTRK )) && ( ptsTrack->tkNextEventDue < tkNext ))
               {
...
//
static dboolean GetTrackEvent( INTRACKSTATE *ptsTrack, PTEMPEVENT pteTemp )
   {
    DWORD   idx;
    BYTE    byByte;
    UINT    dwEventLength;
    ...
    //
    if (( ptsTrack->fdwTrack & ITS_F_ENDOFTRK ) || ( !ptsTrack->dwLeftInBuffer && !ptsTrack->dwLeftOnDisk ))
        return( true );
    ...
    if (( byByte == MIDI_SYSEX ) || ( byByte == MIDI_SYSEXEND ))
       {
...
//
dboolean RefillTrackBuffer( PINTRACKSTATE ptsTrack )
   {
    ...
    if ( ptsTrack->dwLeftOnDisk )
       {
        ...
        if (( dwResult = SetFilePointer( hInFile,(long)ptsTrack->foNextReadStart,0L, FILE_BEGIN )) == 0xFFFFFFFF )
           {
        ...
        bResult = ReadFile( hInFile, ptsTrack->pTrackStart, ptsTrack->dwLeftInBuffer, &dwBytesRead, NULL );
...
static int AddEventToStreamBuffer( PTEMPEVENT pteTemp, CONVERTINFO *lpciInfo )
   {
    ...
    if (( pteTemp->byShortData[0] == MIDI_SYSEX ) || ( pteTemp->byShortData[0] == MIDI_SYSEXEND ))
       {
    ...
    else
       {
        ...
        pmeEvent->dwEvent = ( pteTemp->pLongData[2] ) | (((DWORD)pteTemp->pLongData[1] ) << 8 ) | (((DWORD)pteTemp->pLongData[0] ) << 16 );
...
static void ShowTrackError( PINTRACKSTATE ptsTrack, LPSTR lpszErr )
   {
    wsprintf( szTemp, "Track buffer offset %lu", (DWORD)(ptsTrack->pTrackCurrent - ptsTrack->pTrackStart));
    DebugPrint( szTemp );
    wsprintf( szTemp, "Track total %lu  Track left %lu", ptsTrack->dwTrackLength, ptsTrack->dwLeftInBuffer );
    DebugPrint( szTemp );
   }
